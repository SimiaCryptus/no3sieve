...
export class Renderer {
    constructor(canvas) {
        ...
        if (!this.ctx) throw new Error('Renderer: 2D context unavailable (canvas blocked or out of memory)');
    ...
    _layer(name, fn) {
        try {
            ...
            try {
                this.ctx.restore();
            } catch (_) { /* unbalanced save() from the failed layer */
            }
        }
    }
    ...
    _drawUnknown(vp, rGen) {
        const ctx = this.ctx;
        const x0 = vp.toScreenX(-rGen - 0.5), x1 = vp.toScreenX(rGen + 0.5);
        const y0 = vp.toScreenY(rGen + 0.5), y1 = vp.toScreenY(-rGen - 0.5);
        ctx.save();
        ctx.beginPath();
    ...
    _drawDensity(vp, ps, opts) {
        ...
        const x0 = box.x0 - apron, y0 = box.y0 - apron;
        const w = (box.x1 - box.x0) + 2 * apron, h = (box.y1 - box.y0) + 2 * apron;
        if (!(w > 0) || !(h > 0)) return;
        ...
        const sw = R.cw * R.stride * vp.zoom, sh = R.ch * R.stride * vp.zoom;
        const ctx = this.ctx;
        ctx.save();
    ...
    _drawGrid(vp) {
        if (vp.zoom < 8) return;
        const ctx = this.ctx, b = vp.visibleBox();
    ...
    _drawRings(vp, rGen) {
        ...
        for (let R = step; R <= rGen; R += step) {
            const x0 = vp.toScreenX(-R - 0.5), x1 = vp.toScreenX(R + 0.5);
            const y0 = vp.toScreenY(R + 0.5), y1 = vp.toScreenY(-R - 0.5);
            ctx.rect(x0, y0, x1 - x0, y1 - y0);
        }
        ctx.stroke();
    }
    ...
    _drawPoints(vp, ps) {
        const ctx = this.ctx, z = vp.zoom, k = ps.k, P = ps.points;
        if (!k) return;
        ...
        if (z >= 2) {
            ...
            for (let i = 0; i < k; i++) {
                const x = P[2 * i], y = P[2 * i + 1];
                const sx = vp.toScreenX(x) | 0, sy = vp.toScreenY(y) | 0;
    ...
    _drawSelection(vp, sel) {
        const ctx = this.ctx;
        const x0 = vp.toScreenX(sel.x0 - 0.5), x1 = vp.toScreenX(sel.x0 + sel.s - 0.5);
        const y0 = vp.toScreenY(sel.y0 + sel.s - 0.5), y1 = vp.toScreenY(sel.y0 - 0.5);
