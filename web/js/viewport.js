...
export class Viewport {
    ...
    cellAt(sx, sy) {
        ...
        const x = Math.round(this.toWorldX(sx)), y = Math.round(this.toWorldY(sy));
    ...
    fitRing(R) {
        ...
        this.cx = 0;
        this.cy = 0;
        this.zoom = Math.min(MAX_ZOOM, Math.max(MIN_ZOOM, Math.min(this.w, this.h) / ((2 * R + 1) * pad)));
    }
    ...
    _sane(where) {
        ...
        log.error(`${where}: camera became non-finite (cx=${this.cx}, cy=${this.cy}, zoom=${this.zoom}); resetting`);
        this.cx = 0;
        this.cy = 0;
        this.zoom = 12;
    }
