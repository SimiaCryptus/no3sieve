...
export class PointStore {
    ...
    _grow(need) {
        if (!Number.isInteger(need) || need < 0) throw new RangeError(`PointStore._grow: bad need ${need}`);
    ...
    addRing(rep) {
        if (!rep || typeof rep !== 'object') throw new TypeError('PointStore.addRing: report must be an object');
        if (!Number.isInteger(rep.r) || rep.r < 0) throw new RangeError(`addRing: bad ring index ${rep.r}`);
        if (rep.r <= this.rGen)
            throw new Error(`addRing: ring ${rep.r} arrived after ${this.rGen} — the stream is out of order`);
        const add = rep.added;
        ...
        this._grow(this.k + m);
        for (let i = 0; i < m; i++) {
            const x = add[2 * i], y = add[2 * i + 1];
            const kk = key2(x, y);         // range-checked; also rejects non-int32
            if (this.occ.has(kk))
                throw new Error(`addRing: duplicate point (${x},${y}) already stored at index ${this.occ.get(kk)}`);
    ...
    snapshot() {
        return {points: this.points.subarray(0, this.k * 2), k: this.k, rGen: this.rGen};
    }
}
...
export class EngineRunner {
    constructor(store, on) {
        ...
        this.running = false;
        this.stats = {
            marks: 0, liveEvents: 0, peakEvents: 0, eventBytes: 0, ms: 0,
            cells: 0, blocked: 0, accepted: 0, satRows: 0, satCols: 0,
        };
        this._t0 = 0;
    ...
    start(cfg) {
        ...
        try {
            ...
            this.worker.onerror = (e) => {
                log.warn(`worker error (${e && e.message ? e.message : 'unknown'}) — falling back to the main thread`);
                this._fallback(cfg);
            };
