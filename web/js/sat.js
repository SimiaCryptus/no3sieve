...
export function rasterize(points, x0, y0, w, h, stride = 1) {
    const cw = Math.ceil(w / stride), ch = Math.ceil(h / stride);
...
export function windowPop(S, cw, ch, cx0, cy0, cx1, cy1) {
    ...
    if (!Number.isInteger(cx0) || !Number.isInteger(cy0) || !Number.isInteger(cx1) || !Number.isInteger(cy1))
...
export function centeredDensityMap(sat, cw, ch, sCells) {
    ...
    if (sCells % 2 === 0)
        log.once(`even-window-${sCells}`, 'info',
            `centered window s=${sCells} is even: the centre is biased LOW (documented in the legend, R13)`);
    const lo = Math.floor((sCells - 1) / 2), hi = Math.ceil((sCells - 1) / 2);
...
/** Scan all s×s placements of a raster: max population, argmax, histogram. */
  export function scanMax(sat, cw, ch, s) {
    if (!Number.isInteger(s) || s < 1) throw new RangeError(`scanMax: s must be a positive integer (got ${s})`);
    let best = -1, ax = 0, ay = 0;
    ...
    if (overflow)
        log.error(`scanMax(s=${s}): ${overflow} window(s) hold more than 2s points — the point set is NOT valid`);
