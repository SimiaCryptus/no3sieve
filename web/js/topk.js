...
export function topWindows(points, r, s, keep = 8) {
    ...
    if (!Number.isInteger(r) || r < 0) throw new RangeError(`topWindows: r must be a non-negative integer (got ${r})`);
    if (!Number.isInteger(s) || s < 1) throw new RangeError(`topWindows: s must be a positive integer (got ${s})`);
    if (!Number.isInteger(keep) || keep < 1) throw new RangeError(`topWindows: keep must be >= 1 (got ${keep})`);
    const N = 2 * r + 1;
    if (s > N) {
...
export function windowPoints(points, x0, y0, s) {
    ...
    if (!Number.isInteger(s) || s < 1) throw new RangeError(`windowPoints: s must be a positive integer (got ${s})`);
    const out = [];
