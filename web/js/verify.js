...
export function verify(points, opts = {}) {
    ...
    if (k > softCap) log.warn(`verify: k=${k} → ~${(k * k / 1e6).toFixed(0)}M pair checks; this will block`);
    ...
    for (let i = 0; i < k; i++) {
        ...
        for (let j = 0; j < k; j++) {
            ...
            if (prev !== undefined) {
                return {
                    ok: false, method: 'primdir-hash', k,
                    triple: [[ax, ay], [points[2 * prev], points[2 * prev + 1]],
    ...
    return {
        ok: true, method: 'primdir-hash + I2', k, rMax,
        ms,
    };
}
...
export function verifyBruteForce(points, maxK = 400) {
    ...
    for (let a = 0; a < k; a++) for (let b = a + 1; b < k; b++) for (let c = b + 1; c < k; c++) {
        ...
        const bx = points[2 * b] - ax, by = points[2 * b + 1] - ay;
        const cx = points[2 * c] - ax, cy = points[2 * c + 1] - ay;
        if (bx * cy - by * cx === 0) {
            return {
                ok: false, method: 'cross-product', triple: [[ax, ay],
                    [points[2 * b], points[2 * b + 1]], [points[2 * c], points[2 * c + 1]]]
            };
        }
    }
