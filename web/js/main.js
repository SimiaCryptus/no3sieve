...
window.no3sieveLog = () => logHistory().map((r) => `${r.lvl}\t${r.ns}\t${r.msg}`).join('\n');
...
const opts = {
    grid: true, rings: true, unknown: true,
    density: true, s: 16, norm: 's', alpha: 0.7,
...
if (typeof ResizeObserver === 'function') new ResizeObserver(guard('resize', resize)).observe(canvas);
...
function readConfig() {
    ...
    if (rMax > 2048)
        log.warn(`R_max=${rMax}: expect a multi-second run and a large event pool`);
...
function readSizes() {
    ...
    for (const p of parts) {
        ...
        if (Number.isInteger(n) && n > 0) sizes.push(n); else bad.push(p);
    }
...
$('ov-norm').onchange = guard('normalization', (e) => {
    if (e.target.value !== 's' && e.target.value !== '2s') throw new RangeError(`unknown normalization "${e.target.value}"`);
...
function drawLegend() {
    ...
    const conv = opts.s % 2 ? 'odd s: exact L∞ ball of radius (s−1)/2'
...
$('btn-topk').onclick = () => {
    ...
    ps.config = cfg;
    if (!ps.k) return;
    const sizes = $('tk-sizes').value.split(',').map((s) => parseInt(s, 10)).filter((s) => s > 0);
    ...
    btn.onclick = async () => {
        ...
        switch (kind) {
            ...
            case 'ndjson':
                ...
                break;
            case 'txt':
                EX.download(`${tag}.txt`, 'text/plain',
            ...
            case 'svg':
                ...
                break;
            case 'curve':
                EX.download(`${tag}_curve.csv`, 'text/csv',
                    EX.toDensityCurveCSV(ps.points, ps.rGen, sizes));
                break;
            case 'manifest':
                EX.download(`${tag}_manifest.json`, 'application/json',
                    EX.toManifest(cfg, {
                        k: ps.k, r_gen: ps.rGen, marks: runner.stats.marks,
                        peak_events: runner.stats.peakEvents, ms: runner.stats.ms,
                        points_sha256: pointsHash(ps.points), verification: rep,
                    }, null));
                break;
        }
...
function pushPermalink() {
    const p = new URLSearchParams({
        cx: vp.cx.toFixed(2), cy: vp.cy.toFixed(2), zoom: vp.zoom.toFixed(4),
        s: opts.s, rmax: cfg.rMax, order: cfg.intraRingOrder,
        ov: [opts.density && 'd', opts.grid && 'g', opts.rings && 'r', opts.unknown && 'u'].filter(Boolean).join(''),
    });
...
function readPermalink() {
    ...
    if (order) {
        ...
        else log.warn(`permalink: unknown intra_ring_order "${order}"; keeping ${$('cfg-order').value}`);
    }
    $('ov-s').value = opts.s;
    $('ov-s-out').textContent = opts.s;
