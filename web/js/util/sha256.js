...
export function sha256(bytes) {
  ...
  if (l > 1 << 28)
    throw new RangeError(`sha256: ${l} bytes is too large for this pure-JS implementation`);
...
export function canonicalJson(v, seen = new Set()) {
  ...
  if (t === 'number') {
    if (!Number.isFinite(v))
      throw new TypeError(`canonicalJson: non-finite number (${v}) is not representable`);
  ...
  if (Array.isArray(v)) out = '[' + v.map((e) => canonicalJson(e, seen)).join(',') + ']';
    ...
    out =
      '{' + keys.map((k) => JSON.stringify(k) + ':' + canonicalJson(v[k], seen)).join(',') + '}';
  }
  seen.delete(v);
  return out;
}
...
export function configHash(cfg) {
  ...
  if (typeof cfg.version !== 'string')
    throw new TypeError('configHash: cfg.version must be a string');
