...
function readLevel() {
    ...
    } catch (_) { /* localStorage throws in sandboxed/partitioned contexts */
    }
    try {
    ...
    } catch (_) { /* WorkerLocation may be restricted */
    }
    return LEVELS.warn;
}
...
export function setLevel(name) {
    if (!has(LEVELS, name)) throw new RangeError(`log: unknown level "${name}" (expected ${NAMES.join('|')})`);
    level = LEVELS[name];
    return level;
}
...
export const I32_MIN = -2147483648, I32_MAX = 2147483647;
...
export function requireNonNegInt(v, name) {
    if (!Number.isInteger(v) || v < 0) throw new RangeError(`${name} must be a non-negative integer (got ${fmt(v)})`);
    return v;
}

export function requireEvenLenPairs(a, name) {
    if (!a || typeof a.length !== 'number') throw new TypeError(`${name} must be an array-like of coordinates`);
    if (a.length % 2 !== 0) throw new RangeError(`${name} has odd length ${a.length} (expected [x,y] pairs)`);
    return a;
}