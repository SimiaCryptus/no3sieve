...
export class Inspector {
    ...
    render(cell, ps, opts) {
        ...
        if (!Array.isArray(cell) || cell.length !== 2
            ...
            || Math.abs(cell[0]) >= (1 << 21) || Math.abs(cell[1]) >= (1 << 21)) {
        ...
        if (opts.density) {
            ...
            lines.push(`c_${opts.s}       ${(D / opts.s).toFixed(4)}   (D/2s = ${(D / (2 * opts.s)).toFixed(4)})`);
...
function blockers(cx, cy, P, k) {
    ...
    for (let i = 0; i < k; i++) {
        ...
        if (prev !== undefined) return [[P[2 * prev], P[2 * prev + 1]], [px, py]];
        seen.set(kk, i);
    }
    return null;
}
...
function windowPop(ps, x, y, s) {
    const lo = Math.floor((s - 1) / 2), hi = Math.ceil((s - 1) / 2);
    let n = 0;
    for (let i = 0; i < ps.k; i++) {
        const px = ps.points[2 * i], py = ps.points[2 * i + 1];
