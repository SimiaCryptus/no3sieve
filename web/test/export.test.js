...
import {
    download, toCSV, toTXTGrid, toJSON, toNDJSON, toManifest, toDensityCurveCSV, toSVG, toPNGBlob,
...
test('toCSV emits metadata by default and bare pairs on request', () => {
    ...
    const shifted = toCSV(PTS, {withMeta: false, origin: [1, 1]}).trim().split('\n');
...
test('toTXTGrid prints the top row first', () => {
    ...
    assert.equal(g, '.#\n#.\n');
    assert.equal(toTXTGrid(Int32Array.from([5, 5]), 0, 0, 2), '..\n..\n', 'out-of-window points drop');
...
test('toJSON embeds config_hash, points_sha256 and the verification report', () => {
    ...
    assert.deepEqual(obj.points, [[0, 0], [3, 1], [-2, 2]]);
