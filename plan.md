...
> `SharedArrayBuffer`. One source tree (`src/core/`) is loaded _unmodified_ by both
...
1. A deterministic, reproducible **sieve engine** that incrementally places lattice
   points in `Z^2` in spiral order such that the placed set `P` is _cap-free in the
   collinear sense_: no three points of `P` are collinear.
...
7. A **GPU compute backend** (WebGL2 baseline, WebGPU opt-in) plus Web Workers /
   `SharedArrayBuffer` for the marking phase — an _execution strategy only_: output
   must stay bit-identical to the single-threaded scalar engine (§4.6, `S6`).

### 0.2 Out of scope (v1)

- Exact/optimal search (SAT solvers, ILP, exhaustive backtracking). We may _compare_
...
an affine notion — but _every scheduling decision_ in the engine is indexed by the
...
- `I2` Corollary: at most 2 points per row, per column, and per diagonal ⇒ for the
  ...
  in the _same_ gauge the traversal uses, so it is checkable incrementally at each
...
- `I5` **Segment closure** (§3.7): before candidate `c` is tested, the ring mask
  reflects _every_ line determined by two points already placed earlier in `≺`,
...
alignment) are all _derived from_ L∞ specifically. Alternative gauges are
configurable but must be _convex and monotone_, and they invalidate the closed
forms above.
...
2. **Gauge alignment with the literature.** The classical no-three-in-line problem
   is posed on axis-aligned `n x n` windows, and an `n x n` window _is_ an L∞ ball
   (§4.3). So the traversal metric, the analysis metric (SAT window scan) and the
   target metric are the _same_ metric: `max_pop(2R+1)` is literally `|P ∩ B_∞(R)|`,
...
_(This section is the one the "L0" typo came from. Two orthogonal notions were
conflated there; separating them is exactly what makes the sieve exact.)_
...
1. **The line parameter `t` — metric-free.** For an ordered pair of placed points
   ...
   **iff** it equals `p + t·d` for an _integer_ `t`: exactly the lattice points of
...
2. **The L∞ index — the metric.** The **L∞ index** of a lattice point is
   `R(·) = ||·||_∞`, i.e. which ring it lives on. All _bookkeeping_ is indexed by
...
_ray_, not of the parameter `t`. `t ↦ ||p + t·d||_∞` is convex but **not** monotone
...
vertex `t*` (§3.3); doing it on the unsplit line would silently discard _future_
...
2. `ringMask` — a `Uint32Array` bitset over the _current_ L∞ sphere only.
...
`||d||_∞`, and by _exactly_ `||d||_∞` outside the convexity vertex, so a "steep"
direction _skips_ `~||d||_∞` rings between hits. Since typical primitive directions
...
three-way result of §3.7(2) when `d` is parallel to a face and the ray lies _in_
...
block a cell later in the _same_ ring, and — critically — **two points both placed
in ring `R` define a new line of their own**. Note the loop iterates `points`
_before_ pushing `c`, so same-ring predecessors are included; this is load-bearing.
...
- the _only_ engine in the small-`n` reference implementation (Phase 1).
...
(it would cost `O(R^2)` per placement). It is an _analysis_ stage run on the finished
...
**Gauge alignment (why this bridge is free).** An axis-aligned `s x s` window _is_
...
The _same_ primitive, evaluated per screen pixel instead of per window size, is the
...
- Aperiodicity probes: 2D autocorrelation of the indicator raster; search for any
  ...
  ("Aperiodic" is a _claim to be tested_, not an assumption.)
- Symmetry probes: the construction with `seedPoints = [[0,0]]` and a symmetric
  ...
  (this would _reduce_ interest in the set and is worth knowing early).
...
point can be collinear. The frontier argument only removes dependencies on _smaller_
...
- **Commit (serial, cheap):** walk the ring in order on one thread; for each
  unmarked cell run the `O(k)` exact check against already-committed points _of this
  ring only_ (an `O(k · |committed in ring|)` correction, tiny in practice since a
...
- **Exact marking, no rasterization heuristics.** Each ray is an _instance_;
  ...
  is what makes the GPU result _equal_, not _approximately equal_, to the scalar
  backend's.
...
- **Readback.** Only the ring perimeter (`8R` texels) is read back per ring for the
  serial commit — the Amdahl bottleneck. Amortize by processing a _band_ of `B`
...
- **The viewer is the contract, the in-browser engine is an accelerator.** The
  ...
  UI must be able to _view a headlessly produced artifact_ with the in-browser
...
- World coordinates **are** lattice coordinates. The camera is
  `{ cx, cy, zoom }` with `zoom` in _pixels per cell_, stored log-scaled;
  `screen = (world - c) * zoom + viewport/2`. Camera math is the _only_ place
...
- A tile payload is a bitset (`8 KB` at lod 0) plus a small header; it is produced
  ...
  run, or in the browser from a loaded artifact, _same code_ — and transferred as a
...
The overlay answers, _for the cell under every pixel_, "how good is the
...
- Because both runtimes call the _same_ `codec/*` encoders over the same typed
...
1. **`verify.js` (external certifier).** Loads only the CSV/`.no3`. For each
   ...
   which is _reported explicitly_ as the offending triple. `O(k^2)` time.
...
3. **Greedy-fidelity check (`I4`).** Replay the traversal order with the slow
   reference engine for `R <= 64` and assert the _same_ set is produced. This catches
   the failure mode "engine is valid but over-blocks", which the collinearity
   verifier alone cannot detect.
4. **Segment-closure regression (`I5`).** A hand-built fixture in which the _only_
...
_around_ the typed arrays, while the typed arrays themselves live outside it — one
more reason the calendar is SoA and not an array of records.
A memory model spreadsheet is a Phase-2 deliverable _before_ the code is written.
...
**Accept:** output _identical_ to `reference` for `R <= 64` (differential test);
`>= 20x` faster than reference at `R = 128`.

### M4 — Calendar backend (2 wk) — _the core deliverable_
...
regression (§7.4) passes and _fails_ when the `{(c1,c2)}` family is deliberately
disabled.
...
inspector, permalinks, Canvas2D fallback. Views _artifacts only_ — no in-browser
engine yet.
...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| R1  | Spiral-greedy density is sub-linear (`α < 1`), so the set is _not_ "relatively maximal"                                                                                              | **High**   | High     | Treat measurement of `α` as the deliverable, not a hoped-for `α=1`; add `intraRingOrder`/`ringMetric`/`seedPoints` sweeps and a v2 randomized-restart mode as the follow-up experiment                                                                                                                                   |
...
| R8  | **Segment-closure omission** — the `{(c1,c2)}` pair family of §3.7 is skipped, silently producing _invalid_ sets                                                                     | Med        | **High** | §3.7 is normative; dedicated regression fixture (§7.4) that must fail when the family is disabled; `--paranoid` exact check on every commit in CI; differential vs `reference` at every milestone                                                                                                                        |
...
| R12 | UI scope creep displaces the research deliverable                                                                                                                                    | **High**   | High     | UI milestones sequenced _after_ M5; headline artifact must be produced headlessly; §0.2 bans the editor/framework rabbit holes                                                                                                                                                                                           |
...
3. Is a _bounded_ variant (`n x n` greedy with the same order) meaningfully better
...
4. Euclidean ring intersection: is the closed-form worth it, or should
   ...
   _and_ the window alignment of §4.3, which is a bad exchange.)
...
7. Should the browser _run_ the engine at all, or only view artifacts? (Leaning:
...
- [ ] Every published point set ships with a passing certificate from `verify.js`
      _and_ an independent brute-force check where size permits.
...
- A UI _editor_ that mutates a certified run in place (see §5.7: what-if edits live
...
Every pair of points placed _within the same segment_ also determines a line, and
...
dropping it produces _invalid_ sets, not merely sub-optimal ones. Restated as a
rule: _for all arc cells set at the current **L∞** radius, close over **all pairs**
inside that radius before advancing._ ("L0" in earlier drafts is the typo of the
...
1. **Placement inside a segment is a fixpoint, not a filter.** A candidate may be
   ...
   every point committed so far _in this segment_, and immediately apply the
...
2. **Same-ring chords have a degenerate case.** A line through two points of the
   ...
   is collinear with a face of the square, in which case it contains the _entire
   face_. That is precisely the "two points already in column `x = R`" situation,
...
a configuration where a same-ring pair is the _sole_ blocker of a later same-ring
cell (see R8).
