...
export function toCSV(points, { origin = [0, 0], withMeta = true } = {}) {
  ...
  for (let i = 0; i < k; i++) {
    const x = points[2 * i] - origin[0],
      y = points[2 * i + 1] - origin[1];
    rows.push(
      withMeta ? `${x},${y},${i},${linfIndex(points[2 * i], points[2 * i + 1])}` : `${x},${y}`
    );
  }
  return rows.join('\n') + '\n';
}
...
export function toTXTGrid(points, x0, y0, s) {
  ...
  if (!Number.isInteger(s) || s < 1)
    throw new RangeError(`toTXTGrid: s must be a positive integer (got ${s})`);
...
export function toNDJSON(ringReports) {
  if (!Array.isArray(ringReports))
    throw new TypeError('toNDJSON: expected an array of ring reports');
...
export function toPNGBlob(points, r) {
  ...
  const N = 2 * r + 1;
  if (N > MAX_IMAGE_SIDE)
    throw new RangeError(`toPNGBlob: ${N}x${N} exceeds the ${MAX_IMAGE_SIDE} px canvas limit`);
  ...
  return new Promise((res, rej) =>
    cv.toBlob(
      (b) => (b ? res(b) : rej(new Error('toPNGBlob: canvas.toBlob returned null'))),
      'image/png'
    )
  );
}
...
export function toSVG(points, r, cell = 4) {
  ...
  if (!Number.isFinite(cell) || cell <= 0)
    throw new RangeError(`toSVG: cell must be positive (got ${cell})`);
  const N = 2 * r + 1,
    S = N * cell;
  const k = points.length / 2;
  const parts = [
  ...
  for (let i = 0; i < k; i++) {
    ...
    if (x < 0 || y < 0 || x >= N || y >= N) continue;
    parts.push(
      `<rect x="${x * cell}" y="${(N - 1 - y) * cell}" width="${cell}" height="${cell}" fill="#000"/>`
    );
  }
  parts.push('</svg>');
  return parts.join('\n');
