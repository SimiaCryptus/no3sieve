  ...
  alloc(px, py, dx, dy, t, s, tEnd) {
    ...
    if (this.free.length) i = this.free.pop();
      ...
      if (this.n === this.cap) {
        ...
        this.alive = na;
        log.debug(
          `slab grew to ${this.cap} events (${(this.buf.byteLength / 1048576).toFixed(1)} MB)`
        );
      }
      i = this.n++;
    }
...
export class Calendar {
  constructor(band = 64) {
    ...
    if (band < 8)
      log.warn(`Calendar: band=${band} is very small; most events will spill to the heap`);
    this.B = band;
    this.base = 0;
  ...
  push(r, ev) {
    if (!Number.isInteger(r))
      throw new TypeError(`calendar.push: ring must be an integer (got ${r})`);
    if (!Number.isInteger(ev) || ev < 0) throw new TypeError(`calendar.push: bad event id ${ev}`);
    this.scheduled++;
    if (r < this.base)
      throw new Error(`calendar: event scheduled into the past (${r} < ${this.base})`);
    if (r < this.base + this.B) this.buckets[r % this.B].push(ev);
      ...
      if (this.heap.size > this.peakHeap) {
        ...
        if (this.peakHeap === 1 << 20)
          log.warn(
            `calendar: overflow heap reached ${this.peakHeap} entries; consider a larger band (B=${this.B})`
          );
      }
    }
  }
  ...
  _migrate() {
    while (this.heap.size && this.heap.peekKey() < this.base + this.B) {
      ...
      if (key < this.base)
        throw new Error(`calendar: heap entry ${key} is behind base ${this.base}`);
  ...
  beginRing(r) {
    ...
    while (this.base < r) {
      ...
      if (b.length)
        throw new Error(`calendar: bucket ${this.base} not drained (${b.length} event(s) left)`);
      this.base++;
