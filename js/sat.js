...
export function maxDensityMap(D, cw, ch, sCells) {
  ...
  const M = new Int32Array(cw * ch);
  // Horizontal: max over qx ∈ [x-hi, x+lo] (clamped to the raster).
  for (let y = 0; y < ch; y++) {
    const row = y * cw;
    for (let x = 0; x < cw; x++) {
      const left = Math.max(0, x - hi);
      const right = Math.min(cw - 1, x + lo);
      let m = D[row + left];
      for (let q = left + 1; q <= right; q++) {
        const v = D[row + q];
        if (v > m) m = v;
      }
      tmp[row + x] = m;
  ...
  // Vertical: max over qy ∈ [y-hi, y+lo] (clamped to the raster).
  for (let x = 0; x < cw; x++) {
    for (let y = 0; y < ch; y++) {
      const top = Math.max(0, y - hi);
      const bottom = Math.min(ch - 1, y + lo);
      let m = tmp[top * cw + x];
      for (let q = top + 1; q <= bottom; q++) {
        const v = tmp[q * cw + x];
        if (v > m) m = v;
      }
      M[y * cw + x] = m;
