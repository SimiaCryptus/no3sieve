...
Two more that are *not* cited and should be:
...
* **[check]** **Bounded-window ("local") relaxations of extremal set problems** — locally Sidon sets, `k`-AP-free within
  ...
  reduces the novelty of the *idea* while leaving the exactness theorem (T2A.7) untouched.
* **[check]** **Minimum size of a saturated / maximal no-three-in-line set.** T9.1 (`Ω(n^{2/3})`) and Q9.8 read like a
  ...
  `n^{1−o(1)}`, C3 follows for *every* ring-monotone order and the project's headline question collapses to
...
* **T4.1 / C4.2 (convexity of `t ↦ ||p+td||_∞`, split at `t*`, monotone schedule keys).** The convexity is one line; the
  *use* — split each determined line into two monotone rays, key by the gauge, feed a Dial bucket queue — is the
  sweep-line / event-queue pattern (Bentley–Ottmann) crossed with a segmented sieve of Eratosthenes. The packaging is
  clean and the correctness trap it exposes (T5.3: discarding by step sign rather than after splitting is *unsound*, not
...
* **T4.4 (three-way ring/line classification, with `ContainedInSide`).** Forced by flat faces; elementary; but the
  observation in P8.5 that the degenerate branch is *hot* (fires up to 4× per ring, deposits `Θ(R)` marks) rather than a
...
* **T8.6 (`T·M = Θ̃(k⁴)`, with the oracle and the calendar as the two endpoints of one curve).** Band-regeneration
  ...
  *same* dial is a genuinely clarifying observation and the most useful single fact in §8 for capacity planning.
* **§18.1 (the field as an SFT; the greedy far field as an orbit in it).** Standard framing; the *questions* it makes
...
1. **T2A.7 — the horizon is exact, not approximate.** `P_W ∩ B(⌊W/2⌋) = P_{W'} ∩ B(⌊W/2⌋)` for `W ≤ W'`. This is the
   keystone. It converts a truncation heuristic into a *lossless* resource dial, gives the cheapest possible full-stack
...
2. **T9.2 — the exponent is controlled by one scalar functional.** `α` is decided by the growth of
   ...
   trivially- *definable* scalar is exactly the kind of reformulation that makes a problem attackable (Q13.1). This is
...
3. **T9.3 + T17.1 — the mean-field is inconsistent at `W = ∞`, and the reason is that `W = 2R` is not a steady state.**
   ...
   T17.1's diagnosis — the classical object is the *diagonal* `W = 2R, R → ∞` of a two-parameter family and therefore
...
4. **§16 — the history census and the seam calculus.** `H(c) = |{p ≺ c : ||p−c||_∞ ≤ W}|`, `θ = H/W²`, the exact
   far-field table (L16.2), and the conclusion (T16.3) that *all* anisotropy of a finite-`W` greedy field is supported
   on the corners of the traversal gauge's unit ball plus the intra-ring branch cut. Then H16.4:
   `δ ∝ 1/θ`, giving spokes of width `Θ(W)`, profile `2/(1+j/W)`, peak contrast `≲2`. This is a *mechanism* for a visual
   ...
   "Ulam spiral features are artefacts of the indexing", which this makes precise and, in one respect, *different* (see
   §3.4).
5. **C21/§18.4 — harvest at `W = s−1`, and the inversion it entails.** The observation that a *larger* horizon makes the
   field *sparser at the harvest scale*, so `W` should be tuned to the target rather than maximised, is
...
6. **C2A.3/C8/P17 — `Θ(R²)` certified samples instead of one.** Not deep, but it is what makes the whole exercise
   *reportable*: every window is a certificate, monotone-in-`s`, requiring no theory to be true.
...
* **§12 "Working set `Θ(R)` bits instead of `Θ(R²)`" and "unbounded operation"** are honestly qualified in §8.4 (A2 does
  *not* beat A1 asymptotically in time, and loses badly in memory) — but the §12 summary table restates the advantages
...
* **"Bit-identical output under any parallel schedule" (T7.3+T7.5)** is a theorem about the mark algebra, not about an
  implementation; the implementation-level claim requires that the *set* of produced marks be a function of the ring
...
C2A.10 ("`c*(W) ≍ W^{α−1}`") is the load-bearing conjecture of the whole horizon programme, and it *looks*
...
* therefore the *only* gap between `c*(W)` (far field) and the classical `c(n)` is **homogeneity of `P_W` itself**;
...
(§3.3 below argues that P16 as *stated* will fail, for a reason §16 itself predicts, and that the criterion needs
...
* **T5.3 as a correctness pattern.** "Outward-only pruning is sound *iff* you split first" — a family of bugs (prune by
  step sign) that produce silently *invalid* output rather than a crash. Worth writing up as a standalone cautionary
...
* **T7.9 (at finite `W` the calendar collapses to a rolling band).** No priority queue, no ray records, `Θ(RW)` bits,
  unbounded in `R`. This is the only place in the cost model where a modelling choice removes an *asymptotic* cost
...
* **The methodological triage.** The explicit `L/T/P/H/C/Q/M` tagging, the "conjectures may not be used as hypotheses"
  ...
  good template for computational-mathematics projects. The saturation check (T3.4) in particular catches *over*
...
| Spoke mechanism (H16.4)                        | mean-field constants               | medium: C15/P24 tests the *mechanism* without the constants |
...
4. **P30 (fit both models to `c*(W)`)** — the methodological warning of §17.2 is the most likely way this project
   reaches a *wrong* published conclusion, and P30 is the antidote.
...
declared a *hard invariant* — **[action]** recompute both rows, or the invariant will fire on first run and be mistaken
for a traversal bug.
...
* **The spoke deficit is a surface term and cannot be made extensive.** Over `B(R)`, `E = Θ(RW³)`; the spiral's four
  ...
  `Θ(W/R)` of the mass" is not a coincidence of the profile — it is forced. This *upgrades* the C10-survives argument
...
* **Jensen inverts the design objective.** If `δ ∝ 1/θ` (H16.4), then total yield `∝ E[1/θ] ≥ 1/E[θ] = 1/2`, with
  equality iff `θ` is constant. Since the *mean* of `θ` is pinned at `2` for every order, **the way to increase a greedy
...
is *predicted*, not anomalous.

Crucially the predicted gap is a **`W`-independent constant**, so it does not touch `α = 1 + lim log c*/log W`.
**[action]** Restate P16's criterion: the two estimators must agree **up to a `W`-independent constant**; it is *drift
of the ratio with `W`* that refutes homogeneity, not the ratio itself. As written, P16 will fire on the first run and be
misread as refuting C2A.10.

This also yields a unifying reframe worth adding to §17:

> The classical (`W = ∞`) greedy set is *entirely boundary layer*. Every cell it decides has a truncated past cone,
...
full; it suffices to prove the one-sided comparison *origin window is at least as dense as the far field, uniformly in
`W`*. Combined with a measured `c*(W) → const`, that alone gives `α = 1`.
...
* The disanalogy §16.5 draws ("Ulam is in the labels, ours is in the set") is the right one and is the reason the
  harvest certificates survive the artefact. Worth stating as a slogan: *the mechanism is an artefact; the points are
  not.*
* But the Ulam mechanism is a *quadratic in the ring index along a ray* — and the spiral's ring index is likewise
  quadratic in the enumeration order. If anything is ever observed on the spokes with *arithmetic* rather than *causal*
...
is *global additive* — a single pair constrains a set of candidates spread over the whole range.
...
local exclusion and saturation — RSA/jamming being the canonical family — characteristically land within a *constant
factor* of the optimum, not a polynomial one. And §17's restored mean-field, which is the only self-consistent
...
Counter-consideration worth recording: the greedy 3-AP precedent is *also* the strongest evidence against the
universality conjectures C18–C20, since the Stanley/ternary sequence is the paradigm of a greedy output with zero
entropy and complete self-similar structure. §9.6 and §18 therefore pull in *opposite* directions and cannot both be
...
* **C21 is not an algorithm and §18.4 half-forgets it.** §18.3 correctly concedes `R₁ ≈ s·f^{-1/2}` is astronomical for
  ...
  `Θ(R²)` field to generate first. **[action]** State plainly that C21's value is as a *test of universality* (and of
...
* **The mean-field constants (`ρ`, `κ`) are unpinned and the documents know it** (§17.3). Fine — but `κ` enters the
  headline `c*_∞ = 1/(2√κ)`, so *no numerical prediction of `c*` should be quoted*. Only the scaling `δ ≍ 1/W` and the
...
* **The spiral is not justified at finite `W`.** T7.9's rolling band works for any order with a bounded lookback,
  ...
  seams. Since the spiral's *only* essential virtue is T2A.7/C2A.8 (exactness at the origin for the classical object),
...
* The update depends only on the last `W` bits, so it is a *deterministic finite automaton*: the far field is **provably
...
* It therefore settles, in the analogous 1D setting: (i) whether C12 (periodicity) is the generic outcome; (ii) whether
  ...
  mean-field of §17 gets the *scaling* right (1D mean-field predicts
...
* Known optima for comparison: the densest `W`-locally-3-AP-free set has density `r_3(W)/W = e^{−Θ(√log W)}`
  (Behrend), which is *polynomially above* the mean-field greedy. If the 1D greedy loses polynomially where the optimum
...
H16.4's `δ ∝ 1/θ` law *exactly*, since 1D admits exact computation. This is the only route to making Q20.3 rigorous that
...
§3.2 shows mean `θ = 2` is a conservation law and `δ ∝ 1/θ` is convex, so *yield is maximised by maximising the variance
of `θ`*. A concrete candidate: visit the sublattice `2Z²` first (raster within it), then the rest. Then
...
constant-factor gain in `c*` is *exactly* what the project is trying to measure, and this is a one-flag experiment on
...
single result the engine could produce**, because it makes the traversal a design parameter of a *construction*, not
...
the *total* seam deficit beyond the surface term, so `2m`-gons buy `O(m)` more spokes each `O(1/m)` as strong.
...
* **Q20.1 (`δ(W) = Θ(W^{-4/3})` or `Θ(W^{-1})`)** — the sharpest single statement of the project's question, now that
  §17 has made `Θ(W^{-1})` the mean-field's answer. Note it is *equivalent* to `α ∈ {2/3, 1}` only through C2A.10;
...
* **C11 / Q13.8 (monotonicity of `c*(W)`)** — a counterexample at small `W` is a finite search and the transfer matrix
  ...
  would show order-dependence can *pay*, which is §4.3's thesis in another coordinate.
...
project from a measurement of a construction into a *better construction*.